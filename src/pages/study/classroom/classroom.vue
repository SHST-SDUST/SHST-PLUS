    ...
    <view>
        <layout title="空教室" top-space>
            <view class="text-center a-flex-space-between a-lmt">
                ...
                >
                    <picker-view-column>
                        <view v-for="(item, index) in query.date" :key="index" class="picker-item">
                            {{ item[1] }}
                        </view>
                    </picker-view-column>
                    <picker-view-column>
                        <view v-for="(item, index) in query.time" :key="index" class="picker-item">
                            {{ item[0] }}
                        </view>
                    </picker-view-column>
                    <picker-view-column>
                        <view v-for="(item, index) in query.floor" :key="index" class="picker-item">
                            {{ item[0] }}
                        </view>
                    </picker-view-column>
                </picker-view>
            ...
            <view v-for="item in classrooms.rooms" :key="item">
                <view class="y-center border-box classroom-row">
                    ...
                    <view class="y-center">
                        <view
                            class="a-dot"
                            :style="{ background: classrooms.now[item] ? '#FF5722' : '#009688' }"
                        ></view>
                        <view class="text-ellipsis">
                            {{ classrooms.now[item] ? classrooms.now[item].name : "无课" }}
                        </view>
                    </view>
                    <view class="y-center">
                        <view
                            class="a-dot"
                            :style="{ background: classrooms.next[item] ? '#FF5722' : '#009688' }"
                        ></view>
                        <view class="text-ellipsis">
                            {{ classrooms.next[item] ? classrooms.next[item].name : "无课" }}
                        </view>
                    </view>
                </view>
                <view class="a-hr"></view>
...
export default {
    ...
    methods: {
        ...
        getTimeArr: function () {
            ...
            const queryDate = [];
            const week = safeDate().getDay();
            for (let i = 0; i < 7; ++i) {
                const weekTemp = week + i;
                const weekStr = formatDate("yyyy/MM/dd", date);
