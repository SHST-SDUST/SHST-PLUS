    ...
    <view>
        ...
        <layout title="查成绩" top-space>
            <view class="a-flex-space-between a-pb-5 a-pt-5">
                <view>请选择学期</view>
                <picker @change="bindPickerChange" :value="picker.index" :range="picker.terms" class="a-link" range-key="show">
        ...
        <view v-if="grade.show">
            ...
            <layout v-if="ad.show">
                <!-- #ifdef MP-WEIXIN -->
                <advertise :ad-select="ad.select" :compatible="6" @error="ad.show = false"></advertise>
                <!-- #endif -->
                <!-- #ifdef MP-QQ -->
...
<script>
    import {generateTerms, computeBackground} from "@/vector/pub-fct.js";
    ...
    export default {
        ...
        methods: {
            bindPickerChange: function(e) {
                ...
                let stuYear = this.picker.terms[index].value;
                let query = (stuYear === "" ? "" : "/" + stuYear);
                this.picker.select = this.picker.terms[index].show;
                this.picker.index = index;
                this.getGradeRemote(query);
            },
            initGrade: function() {
                let stuYear = this.picker.select = this.$store.state.curTerm;
                let query = (stuYear === "" ? "" : "/" + stuYear);
                this.getGradeRemote(query);
            },
